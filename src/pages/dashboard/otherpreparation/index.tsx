...
const OtherPreparation: React.FC = () => {
  const { id } = useParams()
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'otherpreparation')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/otherpreparation`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/otherpreparation`, {
        content: contentText,
      })
...
export default OtherPreparation
