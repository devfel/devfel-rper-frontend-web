...
const VennDiagram: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'venndiagram')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/venndiagram`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/venndiagram`, {
        content: contentText,
      })
