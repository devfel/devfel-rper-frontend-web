...
const InputAndOutput: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'inputandoutput')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/inputandoutput`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/inputandoutput`, {
        content: contentText,
      })
