...
const Acknowledgment: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'acknowledgment')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/acknowledgment`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/acknowledgment`, {
        content: contentText,
      })
