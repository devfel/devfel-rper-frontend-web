...
const FocusGroup: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'focusgroup')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/focusgroup`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/focusgroup`, {
        content: contentText,
      })
