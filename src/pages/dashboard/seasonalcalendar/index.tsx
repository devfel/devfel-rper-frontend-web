...
const SeasonalCalendar: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'seasonalcalendar')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/seasonalcalendar`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/seasonalcalendar`, {
        content: contentText,
      })
