...
const OtherFieldwork: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'otherfieldwork')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/otherfieldwork`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/otherfieldwork`, {
        content: contentText,
      })
