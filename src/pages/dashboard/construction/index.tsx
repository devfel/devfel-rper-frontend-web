...
const Construction: React.FC = () => {
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'construction')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/construction`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/construction`, {
        content: contentText,
      })
