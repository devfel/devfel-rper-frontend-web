...
const PrioritiesElection: React.FC = () => {
  const { id } = useParams()
  ...
  const handleEditingResource = async (readonly: boolean) => {
    const isEditing = await findEditingResource(`${id}`, 'prioritieselection')
    if (isEditing) {
  ...
  const handleRemoveEditingResource = async () => {
    await api.delete(`rpers/resources/${id}/${user.user_id}/prioritieselection`)
  }

  const handleSave = async () => {
    try {
      await api.put(`rpers/${id}/prioritieselection`, {
        content: contentText,
      })
...
export default PrioritiesElection
